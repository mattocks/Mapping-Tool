...
Tool.Landmark = {
	...
	click: function() {
		console.log('click occurred')
			...
			if (!over_point) { // if you didn't click on an existing node
				var labelName = prompt('Enter a label')
				if(!(labelName == null)){
					Tool.Landmark.points.push(new Tool.Landmark.MyPoint(Map.pointer_x(), Map.pointer_y(), 5, labelName))
				}
			}
			//else edit the existing point
	}.bindAsEventListener(Tool.Landmark),

	MyPoint: Class.create({
		initialize: function(x,y,r,label) {
			this.x = x
			this.y = y
			this.r = r
			this.label = label
		...
		draw: function() {
				$C.save()
					...
					//	if (this.mouse_inside()) $C.circ(0, 0, this.r)
						...
						var width = $C.measure_text('Arial', 18, this.label)
						$C.stroke_rect(5,-33,width+10,28)
					//}
				$C.restore()
			...
			if (this.dragging && Mouse.down) {
				...
				/*
				var newLabel = prompt('Rename this label')
				if (newLabel != null) {
					this.label = newLabel
				*/
			}
		...
		mouse_inside_text: function() {
			...
			var top = this.y - 33
			var bottom = this.y - 5
			var truth = Map.pointer_x() > left && Map.pointer_x() < right && Map.pointer_y() > top && Map.pointer_y() < bottom
			return truth
		},
		base: function() {
			this.color = '#200'
		...
		dblclick: function() {
			console.log('DoubleClicked my point with label ' + this.label)
		},
		hover: function() {
			this.color = '#900'
			this.dragging = false
...
Tool.Landmark = {
	text: 'test_text',
	mousedown: function(event) {
		//alert('mousedown happened') $C.draw_text('Arial', 18, 'black', Map.pointer_x(), Map.pointer_y(), 'hi')
		//this.points.push(new Tool.Pen.ControlPoint(x, y, 5, this)?
	}.bindAsEventListener(Tool.Landmark),

	mouseup: function() {
		console.log(Map.pointer_x() + ',' + Map.pointer_y())
		$C.draw_text('Arial', 18, 'black', Map.pointer_x(), Map.pointer_y(), 'hi')
	}.bindAsEventListener(Tool.Landmark),
	...
	Point: Class.create({
		...
		click: function() {
			if (Geometry.distance(this.x, this.y, Map.pointer_x(), Map.pointer_y()) < this.r  && Tool.active!='Landmark') {
				this.color = '#f00'
				console.log('clicked control point')
				this.parent_shape.active = true
