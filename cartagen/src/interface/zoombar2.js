var Zoombar = {
	initialize: function(){
		...
		Zoombar.bottomBound = Zoombar.topBound + 270;
...
Zoombar.zoomLevel = function(i){
	return (Zoombar.bottomBound - i)/Zoombar.step;
}
Zoombar.setPosition = function(){
	Zoombar.pos = Zoombar.bottomBound - 6*Map.zoom*20;
	$('indicator').style.top = Zoombar.pos + 'px';
}
Zoombar.zoombarIsDown = function(e){
	Zoombar.mouseDown = true;
	if(e.pageY > Zoombar.bottomBound+Zoombar.step){
		//console.log('got zoom out');
		Zoombar.pos = Math.min(Zoombar.pos+Zoombar.step, Zoombar.bottomBound-Zoombar.step);
	}
...
Zoombar.mouseIsUp = function(){
	if(Zoombar.mouseDown){
		...
		Zoombar.item = null;
		//console.log(pos);
		Zoombar.pos = Zoombar.snapTo(Zoombar.pos);
		$('indicator').style.top = Zoombar.pos + 'px';
		Map.zoom = Zoombar.zoomLevel(Zoombar.pos)*.05;
		console.log(Map.zoom);
		Glop.trigger_draw();
	}
}
Zoombar.moveIndicator = function(e){
	//console.log('indicator dragged');$('indicator').style.top = Zoombar.pos + 'px';
	if(e.pageY < Zoombar.topBound){
		Zoombar.pos = Zoombar.topBound;
	}
	else if (e.pageY > Zoombar.bottomBound){
		Zoombar.pos = Zoombar.bottomBound-Zoombar.step;
	}
	else{
		Zoombar.pos = e.pageY;
	}
